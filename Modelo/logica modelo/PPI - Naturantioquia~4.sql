...
ALTER TABLE secciones_envios
ADD CONSTRAINT secciones_envios_id_transportista_not_null CHECK (id_transportista IS NOT NULL);
...
ALTER TABLE transportistas
ADD CONSTRAINT transportistas_tipo_nn CHECK (tipo IS NOT NULL);
...
-- Restricciones CHECK para asegurar que las columnas no sean nulas
ALTER TABLE usuarios
ADD CONSTRAINT usuarios_tipo_documento_check CHECK (tipo_documento IS NOT NULL);

ALTER TABLE usuarios
ADD CONSTRAINT usuarios_estado_usuario_check CHECK (estado_usuario IS NOT NULL);

ALTER TABLE usuarios
ADD CONSTRAINT usuarios_sexo_usuario_check CHECK (sexo_usuario IS NOT NULL);

ALTER TABLE usuarios
ADD CONSTRAINT usuarios_rol_usuario_check CHECK (rol_usuario IS NOT NULL);
